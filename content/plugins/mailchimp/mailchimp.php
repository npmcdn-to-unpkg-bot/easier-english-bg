...
function mailchimpSF_signup_submit() {
	...
	foreach($mv as $var) {
		...
		if ($var['req'] == 'Y' && trim($opt_val) == '') {
			$success = false;
			$errs[] = sprintf(__("Трябва да попълниш поне e-mail адрес :)", 'mailchimp_i18n'), esc_html($var['name']));
		}
		else {
			if ($var['tag'] != 'EMAIL') {
				$merge[$var['tag']] = $opt_val;
			...
			if (!$retval) {
				switch($api->errorCode) {
					case '105' :
						...
						break;
					case '214' :
						$msg = __("Ах. Този мейл вече е в списъка с абонирани :)", 'mailchimp_i18n');
						...
						if (!$api->errorCode) {
							...
							$msg .= ' ' . sprintf(__('<a href="%s">Кликни тук за да обновиш профила си.</a>', 'mailchimp_i18n'), "http://$username.$dc.list-manage.com/subscribe/send-email?u=$uid&id=$listId&e=$eid");
						}
					...
					case '270' :
						...
						break;
					case '502' :
						$errs[] = __("Този e-mail адрес е невалиден!", 'mailchimp_i18n');
						break;
					default:
