...
function mailchimp_form_field($var, $num_fields) {
	...
	<input type="text" size="18" maxlength="5" value="" name="'.esc_attr($opt).'" id="'.esc_attr($opt).'" class="mc_input" />';
			...
			case 'email':
				$html .= '
	<input type="email" placeholder="Твоят e-mail адрес..." name="'.esc_attr($opt).'" id="'.esc_attr($opt).'" class="mc_input"/>';
				break;
			case 'url':
			case 'imageurl':
			case 'text':
			...
			default:
				$html .= '
	<input type="text" placeholder="Твоето име..." name="'.esc_attr($opt).'" id="'.esc_attr($opt).'" class="mc_input"/>';
				break;
		}
